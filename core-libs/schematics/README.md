...
2. Run `$ npm run test:schematics`. _NOTE_ that when testing `core-libs/schematics`, the command which to run is `$ npm run test`.
...
- let's say you're working on a constructor deprecation task, in which case you would open the `core-libs/schematics/src/migrations/mechanism/constructor-deprecations/constructor-deprecations_spec.ts`.
- first thing to change is the `MIGRATION_SCRIPT_NAME`. If you're testing a migration task for v3, you would change the value of the `MIGRATION_SCRIPT_NAME` constant to `migration-v3-constructor-deprecations-03` (notice the **v3** in the name). To see the exact name of the migration script, you can go to `core-libs/schematics/src/migrations/migrations.json` and copy-paste the script's name you're testing to the spec file.
...
To start creating the schematics configuration, a developer has to first create a configuration file in  the `core-libs/schematics/src/shared/lib-configs/*` directory.

The objects has to conform to the `SchematicsConfig` interface:

- `library.featureName` - corresponds to the CLI's feature name defined in `core-libs/schematics/src/add-spartacus/schema.json`'s `features.items.enum` array.
...
The finished configuration file needs to imported to `core-libs/schematics/src/shared/schematics-config-mappings.ts`' `SCHEMATICS_CONFIGS` array. `SCHEMATICS_CONFIGS`' order follows the order in which features are sorted in the file explorer's tree.
...
If any validations need to be run before actually upgrading Spartacus, you can use the "migration script" located in `core-libs/schematics/src/migrations/3_0/validate.ts`.
...
This section is for developers who do the release, and it specifies how to manage the versions in `core-libs/schematics/src/migrations/migrations.json`.
