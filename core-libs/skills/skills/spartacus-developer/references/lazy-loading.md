...
The proxy is eager; the implementation isn't. Your component can `inject(ActiveCartFacade)` from the root injector even though the cart code itself is lazy. For the full call-resolution mechanics and the implications for extending facades, see [proxy-facade-pattern.md](proxy-facade-pattern.md).
...
- [proxy-facade-pattern.md](proxy-facade-pattern.md) — how `facadeFactory` resolves a call to a lazy-loaded service, and what it means for extending a facade.
