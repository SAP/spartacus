...
function run_vitest_migrated_tests {
    local projects="${1:-$VITEST_PROJECTS}"
    echo "Running VITEST unit tests for migrated libraries"
    npx nx run-many --target=test --projects="$projects"
    return 0
}

function run_vitest_affected_tests {
    echo "Running VITEST unit tests for affected migrated libraries"
    npx nx affected --target=test --projects="$VITEST_PROJECTS"
    return 0
}

function run_affected_unit_tests {
    echo "Running JASMINE unit tests and code coverage for AFFECTED libraries"
    npx nx affected --target=test --exclude="$EXCLUDE_APPLICATIONS,$JEST_PROJECTS,$VITEST_PROJECTS" -- --no-watch --source-map --code-coverage --browsers ChromeHeadless
...
function run_all_unit_tests {
    if [[ -n "$UNIT_TEST_GROUP_PROJECTS" ]]; then
        ...
        npx nx run-many --target=test --projects="$UNIT_TEST_GROUP_PROJECTS" --exclude="$EXCLUDE_APPLICATIONS,$JEST_PROJECTS,$VITEST_PROJECTS" -- --no-watch --source-map --code-coverage --browsers ChromeHeadless
        ...
        run_vitest_migrated_tests "$UNIT_TEST_GROUP_PROJECTS"
    else
        echo "Running ALL JASMINE unit tests"
        npx nx run-many --all --target=test --exclude="$EXCLUDE_APPLICATIONS,$JEST_PROJECTS,$VITEST_PROJECTS" -- --no-watch --source-map --code-coverage --browsers ChromeHeadless
