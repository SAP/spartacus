...
EXCLUDE_JASMINE=storefrontstyles,schematics,setup,core,storefrontlib
EXCLUDE_JEST=storefrontstyles,schematics,setup

echo "-----"

function run_affected_unit_tests {
    echo "Running VITEST unit tests and code coverage for core and storefrontlib"
    npx nx run core:test -- --coverage
    npx nx run storefrontlib:test -- --coverage
