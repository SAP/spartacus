...
KARMA_NAMES=$(jq -nc \
    ...
    '$a | map(select(. as $p | ($apps + $jestonly) | index($p) | not))')

JEST_NAMES=$(jq -nc \
    --argjson a "$(shard_show_projects test-jest "$AFF_FLAG")" --argjson apps "$EXCLUDED_APPS" \
    '$a | map(select(. as $p | $apps | index($p) | not))')
...
INCLUDE=$(printf '%s' "$MATRIX" | jq -c '.include')
if [[ "$AFFECTED" != "true" ]] && { [[ -z "$INCLUDE" || "$INCLUDE" == "[]" ]]; }; then
