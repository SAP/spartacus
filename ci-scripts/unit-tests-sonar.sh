...
echo "Running unit tests and code coverage for product"
exec 5>&1
...
echo "Running unit tests and code coverage for my-accunt"
exec 5>&1
output=$(ng test my-accunt --sourceMap --watch=false --code-coverage --browsers=ChromeHeadless | tee /dev/fd/5)
coverage=$(echo $output | grep -i "does not meet global threshold" || true)
if [[ -n "$coverage" ]]; then
    echo "Error: Tests did not meet coverage expectations"
    exit 1
...
echo "Running unit tests and code coverage for gigya"
exec 5>&1
output=$(ng test gigya --sourceMap --watch=false --code-coverage --browsers=ChromeHeadless | tee /dev/fd/5)
coverage=$(echo $output | grep -i "does not meet global threshold" || true)
if [[ -n "$coverage" ]]; then
    echo "Error: Tests did not meet coverage expectations"
    exit 1
