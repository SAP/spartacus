...
Every key belongs to some named space (namespace). Each space is encapsulated in a chunk (`i18n.chunks` below). But in order to resolve which key is in which namespace and chunk, a config is needed:
