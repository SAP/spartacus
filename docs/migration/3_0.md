...
- `NotCheckoutAuthGuard` no longer uses `RoutingService`. These services usage was replaced with the corresponding methods from `SemanticPathService` and `Router`.