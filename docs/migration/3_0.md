...
- `anonymousConsentsStoreConfigFactory` was removed. Function has been replaced by `anonymousConsentsStatePersistenceFactory`. It requires param type `AnonymousConsentsStatePersistenceService` and it doesn't return `StateConfig`, only function allows for initialization the synchronization between state and browser storage.
