...
- `LoginFormComponent` no longer uses `ActivatedRoute`, `CheckoutConfigService` and `AuthRedirectService`. The logic using these services was moved to a different component. `LoginFormComponent` no longer has properties `loginAsGuest`, `sub` and the method `ngOnDestroy`.
