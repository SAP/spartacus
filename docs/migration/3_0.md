...
There can be more than one cart entry with the same product code. So now they are referenced by the property `entryNumber` instead of product code in `CartItemListComponent`.

### AddedToCartDialogComponent lost members and closes itself on URL change
`AddedToCartDialogComponent` lost members: 
- `this.increment` - use new `this.numberOfEntriesBeforeAdd` instead
- `this.cartEntry$` - use `this.activeCartService.getLastEntry(productCode)` instead

...and earned new constructor dependency `RoutingService`

Moreover, now the `AddedToCartDialogComponent` closes itself whenever a router navigation happens (i.e. when clicking a link inside the modal).

### ContentPageMetaResolver
`ContentPageMetaResolver` earned a new required constructor dependency `RoutingPageMetaResolver`.

### CmsComponentService
The return type of the method `CmsComponentService.getChildRoutes()` changed from `Route[]` to `CmsComponentChildRoutesConfig`.
