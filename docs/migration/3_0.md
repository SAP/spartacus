...
Kyma module were removed with all it's code. We expose the same functionality through configuration of auth.
...
- `AsmActions` no longer contains actions related to customer agent token. Instead interact directly with `CsAgentAuthService`.
