...
- Attributes `role="tablist"` and `class="cx-nav"` were added to `ul`.
- Attribute `class="cx-nav-item"` has been added to `li` elements.
- Clickable tabs were replaced from `a` tag to `button` with `cx-nav-link` class.
