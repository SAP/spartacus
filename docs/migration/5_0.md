...
- Class no longer implements `AfterContentChecked` interface.
- Subject `reviewsTabAvailable` has been removed.
- Private method `getReviewsComponent` has been removed.

#### Template changes

- Now `button.btn.btn-link.cx-action-link` is rendered based on `product.numberOfReviews` property to improve stability of component and avoid flakiness of e2e tests.