...
- `ImportEntriesDialogComponent` extends `DialogComponent` now, so `close` method has been moved there.
- Event `click` is handled by `handleClick` method.
- `focusConfig.autofocus` is set to `'button.btn-action'`.
- `focusConfig.focusOnEscape` is set to `false`.

#### Template Changes

- `div.cx-import-entries-dialog` element has `*ngIf="summary$ | async as summary"` condition now. Additionally, it supports `esc` key event now.  
- `div.cx-import-entries-container` has new attribute `tabindex="-1"`.
- `cx-import-to-new-saved-cart-form` has new attribute `tabindex="0"`.
- `cx-import-entries-summary` has new directive `[cxFocus]="focusConfig"`.

### ImportEntriesSummaryComponent

#### Template Changes

- `div.cx-import-entries-summary-status`  has new attribute `tabindex="-1"`.

### SavedCartFormDialogComponent

- `SavedCartFormDialogComponent` extends `DialogComponent` now, so `close` method has been moved there.

### ReplenishmentOrderCancellationDialogComponent

- `ReplenishmentOrderCancellationDialogComponent` extends `DialogComponent` now, so `close` method has been moved there.

### ProduceImageZoomDialogComponent

- `ProduceImageZoomDialogComponent` extends `DialogComponent` now, so `close` method has been moved there.

#### Template Changes

- `div.cx-image-zoom-dialog` supports `esc` key event now.

### AnonymousConsentsDialogComponent

- `AnonymousConsentsDialogComponent` extends `DialogComponent` now, so `close` method has been moved there.
