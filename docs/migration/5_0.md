...
- `div.row > div.col-md-12.col-lg-9 > ng-container` will appear only if  `supportedDeliveryModes$` is loaded with length and `isSetDeliveryModeBusy$` is loaded with falsy `continueButtonPressed` flag is falsy.
