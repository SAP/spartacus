...
This page contains all the technical changes introduced in 5.0 release.
Please note the automated migration process might yield some false-positive results. For example, after running the Spartacus upgrade schematics, you might see a comment in your code, where Spartacus is informing you about a breaking change, even though there is none. This is a false-positive, which likely happened because Spartacus uses the same API method / property name. In cases like these, it is recommended to just ignore the comment and remove it.
