...
- Added new value `STOCK_NOTIFICATION` to `LAUNCH_CALLER`

### AddedToCartDialogEventListener

- Removed `ModalService` from constructor
- Added `LaunchDialogService` to constructor
- Logic for `openModal` and `closeModal` methods was changed to support toggling dialog component from `LaunchDialogService`
- Removed `modalRef` property
...
- Removed `ModalService` from constructor
- Added `LaunchDialogService` to constructor
- Component cycle hooks: `OnInit`, `OnDestroy` have been added
- Removed `modalIsOpen` property
- Removed `dialog` property
...
- Removed `suggestedAddressModalRef` property 
...
- Added `ViewContainerRef` to constructor
- Removed `ModalService` from constructor
- Added `ElementRef` property
- `openModal` is now using `openDialog` method from `LaunchDialogService`
...
- Added `focusConfig`
- `dismissModal` is now using `closeDialog` from `LaunchDialogService`
...
- Removed `NgbModule` from `imports` section
...
- Added `LaunchDialogService` to constructor
- Added `ViewContainerRef` to constructor
- `openDialog` is now using `openDialog` method from `LaunchDialogService`

### StockNotificationtModule
- Added `KeyboardFocusModule` to `imports` section
- Added `provideDefaultConfig(defaultStockNotificationLayoutConfig)` to `providers` section
...
- Added `ElementRef` to constructor
- `close` is now using `closeDialog` method from `LaunchDialogService` to control dialog dismissing behaviour
- Component cycle hook: `OnInit` has been added
