...
- Subject `reviewsTabAvailable` has been renamed to "areReviewsAvailable$".
...
- Return type of `getReviewsComponent` has been changed from `Element` to `HTMLElement | undefined`.
- Return type of `getTabsComponent` has been changed from `Element` to `HTMLElement | undefined`.
