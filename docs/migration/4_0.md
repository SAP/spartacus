...
- `OccUserAdapter` was removed. Instead please use `OccUserAccountAdapter` and `OccUserProfileAdapter` from `@spartacus/user` lib.
- The `remove` method was removed. Use `close` method instead.

#### UserAdapter

- `UserAdapter` was removed. Instead please use `UserAccountAdapter` and `UserProfileAdapter` from `@spartacus/user` lib.
- The `remove` method was removed. Use `close` method instead.

#### UserConnector

- `UserConnector` was removed. Instead please use equivalents from `UserAccountConnector` and `UserProfileConnector` from `@spartacus/user` lib.
- The `remove` method now returns `close` method from adapter (name change).

#### OccEndpoints

- Endpoint `user` was removed. For replacement please use it from `@spartacus/user/account`.
- Endpoints `titles`, `user`, `userRegister`, `userForgotPassword`, `userResetPassword`, `userUpdateLoginId`, `userUpdatePassword`, `userUpdateProfile`, `userCloseAccount` were removed. For replacement please use them from `@spartacus/user/profile`.

#### UserService

- `get`, `load` methods were removed, instead please use `UserAccountFacade.get()` from `@spartacus/user`.
