...
8. Now let's focus on replacing deprecated Spartacus grouping modules
    ...
    7. Migrate `OccModule`
        - add `AsmOccModule` from `@spartacus/core` to imports in `SpartacusFeaturesModule`
        - add `CartOccModule` from `@spartacus/core` to imports in `SpartacusFeaturesModule`
        - add `CheckoutOccModule` from `@spartacus/core` to imports in `SpartacusFeaturesModule`
        - add `ProductOccModule` from `@spartacus/core` to imports in `SpartacusFeaturesModule`
        - add `UserOccModule` from `@spartacus/core` to imports in `SpartacusFeaturesModule`
        - remove usage of `OccModule` from your app

    8. Migrate `EventsModule`
        - add `CartPageEventModule` from `@spartacus/storefront` to imports in `SpartacusFeaturesModule`
        - add `PageEventModule` from `@spartacus/storefront` to imports in `SpartacusFeaturesModule`
        - add `ProductPageEventModule` from `@spartacus/storefront` to imports in `SpartacusFeaturesModule`
