...
### PromotionService
PromotionService is now an abstract class.  It has three implementations ready to use, depending on the source for the promotions: `CheckoutPromotionService`, `OrderPromotionService`, `CartPromotionService`.  Use the appropriate concrete iimplementation wether you need promotions from the cart, the order or the checkout(order confirmation)

The "promotionLocation" is still passed down to the CartItemComponent because it's used for other purposes than just the promotions.   It's used to declare the context in which the cart item is displayed. 
...
constructor parameter `PromotionService` now refers to the abstract service class `PromotionService` and the injection of the concrete implementation iis configured in cms component config.
It uses the implementation `CheckoutPromotionService`
...
constructor parameter `PromotionService` now refers to the abstract service class `PromotionService` and the injection of the concrete implementation iis configured in cms component config.
It uses the implementation `CartPromotionService`
...
### CartItemListComponent
To allow `CartItemListComponent` to display the list of product promotions, these have to be fetched beforehand and they need to be provided to CartItemListComponent via the new input parameter `promotions`.  The list of promotions can be obtained by a `PromotionService` implementation class via the function `getProductPromotionForAllEntries`. 

The promotions related to an entry are passed down to `CartItemComponent` that will render them along with the entry.

Even if it is not used directly to fetch promitions, the `promotionLocation` should still be used because the location is used for other things than just promotions.
