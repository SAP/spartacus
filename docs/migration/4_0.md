...
Some services are now available through facades.  faccades should be used instead.  The main advantage to use facades instead of their service implementationo is that the facades support lazy loading.  Facades are imported from `@spartacus/checkout/root`
...
Protected method`getCartEndpoint` has been removed. There are new methods: `getClearDeliveryModeEndpoint`, `getLoadCheckoutDetailsEndpoint`, `getPlaceOrderEndpoint`, `getRemoveDeliveryAddressEndpoint`.
...
| AddressVerificationState | @spartacus/core | @spartacus/checkout/core | - | 
