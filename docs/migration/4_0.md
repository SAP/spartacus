...
### PromotionService
PromotionService is now an abstract class.  Instead, use one of the concrete implementations: `CheckoutPromotionService`, `OrderPromotionService`, `CartPromotionService`.
The concrete implementations are used instead of the concept of the "promotino location".
Use the appropriate concrete iimplementation wether you need promotions from the cart, the order or the checkout(order confirmation)
