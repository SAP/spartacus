...
- `USER_SERIALIZER` was removed. Instead please use `USER_SERIALIZER` from `OccUserProfileAdapter` from `@spartacus/user` lib.
...
- Methods `update`, `requestForgotPasswordEmail`, `resetPassword`, `updateEmail`, `updatePassword`, `remove` were removed. Instead please use equivalents from `UserAccountConnector` and `UserProfileConnector` from `@spartacus/user` lib.
- The `remove` method now returns `close` method from adapter (name change).

#### OccEndpoints

- Endpoints `userForgotPassword`, `userResetPassword`, `userUpdateLoginId`, `userUpdatePassword`, `userUpdateProfile`, `userCloseAccount` were removed. For replacement please use them from `@spartacus/user/profile`.
