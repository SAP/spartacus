...
- `page-meta.service.ts`
  We made the `resolverMethods` protected, as it was public before for no good reason. 
  We increased exensibility by making the resolve method protected. The `resolve` method will call individual resolvers from page resolve classes by the hand of the list of `resolverMethods`, so that page resolvers no longer have to do the heavy lifting of resolving all data. This increases exensibility. 
- `cart-page-meta.resolver.ts`  
  The deprecated method `resolve` was removed with 2.0. This method is no longer supported as individual resolve methods (`resolveTitle`, `resolveRobots`) are called automatically in the `PageMetaService`. The individual resolvers (`resolveTitle`, `resolveRobots`) no longer receive arguments, but use local observables (`cms$`) to resolve the required data.
- `checkout-page-meta.resolver.ts`  
  The deprecated method `resolve` was removed with 2.0. This method is no longer supported as individual resolve methods (`resolveTitle`, `resolveRobots`) are called automatically in the `PageMetaService`. The individual resolvers (`resolveTitle`, `resolveRobots`) no longer receive arguments, but use local observables (`cart$`) to resolve the required data.
- `content-page-meta.resolver.ts`
  The deprecated method `resolve` was removed with 2.0. This method is no longer supported as individual resolve methods (`resolveTitle`, `resolveBreadcrumbs`) are called automatically in the `PageMetaService`. The individual resolvers (`resolveTitle`, `resolveBreadcrumbs`) no longer receive arguments, but use local observables (`cms$`) to resolve the required data.
- `category-page-meta.resolver.ts`
  The deprecated method `resolve` was removed with 2.0. This method is no longer supported as individual resolve methods (`resolveTitle`, `resolveBreadcrumbs`) are called automatically in the `PageMetaService`. The individual resolvers (`resolveTitle`, `resolveBreadcrumbs`) no longer receive arguments, but use local observables (`searchPage$`) to resolve the required data.
- `search-page-meta.resolver.ts`
  The deprecated method `resolve` was removed with 2.0. This method is no longer supported as individual resolve methods (`resolveTitle`) are called automatically in the `PageMetaService`. The individual resolvers (`resolveTitle`) no longer receive arguments, but use local observables (`query$`) to resolve the required data.
- `find-product-page-meta.resolver.ts`
  The deprecated method `resolve` was removed with 2.0. This method is no longer supported as individual resolve methods (`resolveTitle`, `resolveBreadcrumbs`) are called automatically in the `PageMetaService`. The individual resolvers (`resolveTitle`, `resolveBreadcrumbs`) no longer receive arguments, but use local observables (`total$`) to resolve the required data. The scoring functionality was refactored.
