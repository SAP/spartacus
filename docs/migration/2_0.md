...
The collection of page meta data has been changed. Previously, each meta resolver implementation has been responsible to resolving all data, by implemeting the abstract `resolve` method from the abstract `PageMetaResolver`. To allow more flexibility and simplfiy cusomisations, the individual implementations no longer implement the resolve method. Only an implementationf or a specific resolver is required, i.e. the `PageTitleResolver`. The `PageMetaService` will call all resolvers of the page resolver classes. The `PageMetaService` does this by the hand of the registered `resolverMethods`. You can further extend the list of resolverMethods without changing the impleemntation of `PageMetaService.resolve`.

These changes have been introduced in version 1.3 under a feature flag, and became standard in 2.0. The `FeatureConfigService` was used for this feature flag, and has been dropped from all constructors with version 2.0. This change will be migrated automatically.

The individual changes for 2.0 per class are:

- `PageMetaService`  
  The `resolverMethods` became protected, as it was public before for no good reason. The `resolve` method will call individual resolvers by iterating over the `resolverMethods`.
