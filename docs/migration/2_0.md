...
We are dropping `cart` store feature (`CART_DATA`, `StateWithCart`, `CartsState`, `CART_FEATURE`, `CartState`) along with it's selectors (`CartSelectors`). Instead we recommend working with `ActiveCartService` which uses under the hood new `carts` store feature (previously under `multi-cart` key in ngrx). It allows us to support more carts (eg. wishlist, saved carts).
