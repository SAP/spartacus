...
`authentication.kyma_enabled` configuration option has been removed. Just remove this property, as kyma is now enabled by importing `KymaModule`.
...
Method `addDynamicAttributes` now have different order of parameters. Properties (now `cmsRenderingContext`) are now the last parameter and are typed.

### CmsActions and CmsSelectors

`CmsActions` now have only parameter: `payload`. Other parameters are now part of `payload`.

`getComponentState`, `getComponentEntities`, `componentStateSelectorFactory` and `componentSelectorFactory` were removed from `CmsSelectors`.
...
Skip link default configuration was simplified.
...
| `ProductImagesComponent.isThumbsEmpty` variable removed | Use `thumbs$` observable instead | - |
