...
- `ConfigurableRoutesService` no longer uses `UrlMatcherFactoryService`, but it's renamed counterpart `UrlMatcherService`.
- `ExternalRoutesService` no longer uses `UrlMatcherFactoryService`, but it's renamed counterpart `UrlMatcherService`.
