...
|-------|-------------|---------|
| RegisterComponent(auth: AuthService, authRedirectService: AuthRedirectService, userService: UserService, globalMessageService: GlobalMessageService, fb: FormBuilder); | RegisterComponent(userService: UserService, globalMessageService: GlobalMessageService, fb: FormBuilder, router: RoutingService, anonymousConsentsService: AnonymousConsentsService, anonymousConsentsConfig: AnonymousConsentsConfig) | The constructor now uses new dependencies. `RegisterComponent(userService: UserService, globalMessageService: GlobalMessageService, fb: FormBuilder, router: RoutingService, anonymousConsentsService: AnonymousConsentsService, anonymousConsentsConfig: AnonymousConsentsConfig)` |
...
- `RegisterComponent` no longer uses `AuthService` and `AuthRedirectService`. Also `RoutingService`, `AnonymousConsentsService` and `AnonymousConsentsConfig` are now required parameters.
