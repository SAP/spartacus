...
- `QualtricsLoaderService` now requires Angulars' core `RendererFactory2` and no longer uses the `QualtricsConfig`. The service has been refactored for better extensibility and reuse. 
- `QualtricsComponent` now requires the `QualtricsConfig` to load the global qualtrics deloyments script. This was a dependency in the `QualtricsLoaderService` in 1.x. 
