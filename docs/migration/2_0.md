...
[TODO: add message - configure only optimized loading scopes for product data]
...
- `ProductPageMetaResolver` no longer uses `FeatureConfigService`.
- `CurrentProductService` no longer uses `FeatureConfigService`.
