...
`AuthStorageService.getToken()` returns an in-memory `BehaviorSubject` that emits
synchronously — `take(1)` always completes immediately with the current token. Anonymous
users get an empty token object `{}` — no `Authorization` header is sent.
...
and sets the proxy target dynamically. The browser always calls `/bff/api`
...
`x-app-custom` header. Button-triggered — no call on init.
...
`GET /occ/v2/basesites` on the OCC backend. Button-triggered.
...
> **Angular PendingTasks note:** BFF calls must be triggered by user interaction,
> not on component init. Use `Subject` + `switchMap` + `takeUntilDestroyed()` to
> avoid leaving Angular pending tasks dangling during SSR hydration.
...
Spread into `app.module.ts`: `providers: [privateProviders, bffExampleProviders]`

---

## Vivaldi BFF changes

### 12. `apps/bff/env.d.ts` *(modify)*
...
### 13. `apps/bff/vivaldi.apis.ts` *(modify)*
...
### 14. `packages/contracts/bff/destinations.ts` *(modify)*
...
### 15. `apps/bff/src/api/routers/occ.ts` *(new file)*

tRPC router for OCC proxy procedures. Each procedure forwards the request to OCC via
`ctx.execute.http` and the `occ_v2` destination. The caller's `Authorization` header
is forwarded unchanged so OCC authentication works transparently.
...
with the desired OCC path, forwarded headers, and `ctx.destinations.occ.v2()`.

---

### 16. `apps/bff/src/api/routers/root.ts` *(modify)*
...
### 17. `apps/bff/.env` *(local dev only)*
...
self-signed certs. Set this in the CCv2 BFF VariableSet for deployed environments.
...
cd /path/to/bff-repo
OCC_BASE_URL=https://your-occ-host npx nx serve bff
