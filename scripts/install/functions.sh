...
function add_epd_visualization {
    if [ "$ADD_EPD_VISUALIZATION" = true ] ; then
        ng add @spartacus/epd-visualization --baseUrl ${EPD_VISUALIZATION_BASE_URL} --skip-confirmation --no-interactive
    fi
}
...
function add_spartacus_csr {
    ...
    if [ "$BASE_SITE" = "" ] ; then
      ng add @spartacus/schematics@${SPARTACUS_VERSION} --skip-confirmation --overwriteAppComponent --baseUrl ${BACKEND_URL} --occPrefix ${OCC_PREFIX} --urlParameters ${URL_PARAMETERS} --no-interactive
    else
      ng add @spartacus/schematics@${SPARTACUS_VERSION} --skip-confirmation --overwriteAppComponent --baseUrl ${BACKEND_URL} --occPrefix ${OCC_PREFIX} --baseSite ${BASE_SITE} --urlParameters ${URL_PARAMETERS} --no-interactive
    fi
    add_feature_libs
    add_b2b
    add_cdc
...
function add_spartacus_ssr {
    ( cd ${INSTALLATION_DIR}/${1}
    if [ "$BASE_SITE" = "" ] ; then
      ng add @spartacus/schematics@${SPARTACUS_VERSION} --overwriteAppComponent --baseUrl ${BACKEND_URL} --occPrefix ${OCC_PREFIX} --urlParameters ${URL_PARAMETERS} --ssr --no-interactive --skip-confirmation
    else
      ng add @spartacus/schematics@${SPARTACUS_VERSION} --overwriteAppComponent --baseUrl ${BACKEND_URL} --occPrefix ${OCC_PREFIX} --baseSite ${BASE_SITE} --urlParameters ${URL_PARAMETERS} --ssr --no-interactive --skip-confirmation
    fi
    add_feature_libs
    add_b2b
    add_cdc
...
function add_spartacus_ssr_pwa {
    ( cd ${INSTALLATION_DIR}/${1}
    if [ "$BASE_SITE" = "" ] ; then
      ng add @spartacus/schematics@${SPARTACUS_VERSION} --overwriteAppComponent --baseUrl ${BACKEND_URL} --occPrefix ${OCC_PREFIX} --urlParameters ${URL_PARAMETERS} --ssr --pwa --no-interactive --skip-confirmation
    else
      ng add @spartacus/schematics@${SPARTACUS_VERSION} --overwriteAppComponent --baseUrl ${BACKEND_URL} --occPrefix ${OCC_PREFIX} --baseSite ${BASE_SITE} --urlParameters ${URL_PARAMETERS} --ssr --pwa --no-interactive --skip-confirmation
    fi
    add_feature_libs
    add_b2b
    add_cdc
