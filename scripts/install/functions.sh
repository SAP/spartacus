...
function create_shell_app {
    ...
    if [[ "$(compareSemver "$current_npm_version" "11.0.0")" -lt 0 ]]; then
        echo "ERROR: npm version $current_npm_version is too old. Please install npm@11 or higher: npm install -g npm@11" >&2
        exit 1
    fi
...
function add_opf {
    if [ "${ADD_OPF}" = true ] ; then
        ng add @spartacus/opf@${SPARTACUS_VERSION} --opf-base-url ${OPF_BASE_URL} --commerce-cloud-public-key ${OPF_CLIENT_PUBLIC_KEY} --enable-get-active-configurations-access-code-header ${OPF_ACCESS_CODE_HEADER_ENABLED} --skip-confirmation --no-interactive
    fi
...
function start_ssr_unix {
    if [ -z "${SSR_PORT}" ]; then
        ...
        ( cd ${INSTALLATION_DIR}/${SSR_APP_NAME} && export PORT=${SSR_PORT} && export NODE_TLS_REJECT_UNAUTHORIZED=0 && export NG_ALLOWED_HOSTS=${SSR_ALLOWED_HOSTS:-localhost} && "${SPARTACUS_BIN}/pm2" start --name "${SSR_APP_NAME}-${SSR_PORT}" dist/${SSR_APP_NAME}/server/$serverFileName )
    fi
}
