import { SPARTACUS_CUSTOMER_TICKETING } from '@spartacus/schematics';

export const CUSTOMER_TICKETING_FOLDER_NAME = 'customer-ticketing';
export const CUSTOMER_TICKETING_MODULE_NAME = 'CustomerTicketing';

export const CUSTOMER_TICKETING_FEATURE_NAME_CONSTANT = 'CUSTOMER_TICKETING_FEATURE';
export const SPARTACUS_CUSTOMER_TICKETING_ROOT = `${SPARTACUS_CUSTOMER_TICKETING}/root`;
export const SPARTACUS_CUSTOMER_TICKETING_ASSETS = `${SPARTACUS_CUSTOMER_TICKETING}/assets`;
export const CUSTOMER_TICKETING_TRANSLATIONS = 'customerTicketingTranslations';
export const CUSTOMER_TICKETING_TRANSLATION_CHUNKS_CONFIG = 'customerTicketingTranslationChunksConfig';
export const CUSTOMER_TICKETING_SCSS_FILE_NAME = 'customer-ticketing.scss';
