...
export class OccConfiguratorVariantSerializer
  ...
  convert(
    ...
    source?.groups?.forEach((group) => {
      const resultGroups = resultTarget.groups;
      if (resultGroups) {
        this.convertGroup(group, resultGroups);
      }
    });
  ...
  convertGroup(
    source: Configurator.Group | undefined,
    occGroups: OccConfigurator.Group[] | undefined
  ) {
    if (source && occGroups) {
  ...
  convertAttribute(
    attribute: Configurator.Attribute | undefined,
    occAttributes: OccConfigurator.Attribute[] | undefined
  ): void {
    if (attribute && occAttributes) {
