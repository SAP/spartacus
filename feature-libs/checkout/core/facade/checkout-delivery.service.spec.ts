...
describe('CheckoutDeliveryService', () => {
  ...
  describe('with commands and queries enabled', () => {
    ...
    // TODO:#13888 Remove test when removing set delivery address process
    it('should still be able to observe set delivery address process', (done) => {
      let loaderState: StateUtils.LoaderState<any>[] = [];
      service
        .getSetDeliveryAddressProcess()
        .pipe(take(3))
        .subscribe((data) => {
          loaderState.push(data);
          if (loaderState.length === 3) {
            expect(loaderState).toEqual([
              ...
              jasmine.objectContaining({ loading: true }),
              jasmine.objectContaining({ loading: false, success: true }),
            ]);
            done();
          }
        });

      service.setDeliveryAddress({ ...address, id: 'addressId' });
    });

    // TODO:#13888 Remove test when removing set delivery address process
    it('should still be able to observe set delivery address process failure', (done) => {
      spyOn(checkoutDeliveryConnector, 'setAddress').and.returnValue(
        ...
        .subscribe((data) => {
          ...
          if (loaderState.length === 3) {
            expect(loaderState).toEqual([
              ...
              jasmine.objectContaining({ loading: true }),
              jasmine.objectContaining({ loading: false, error: true }),
            ]);
            done();
          }
        });
