...
export class CheckoutEffects {
          ...
          .pipe(
            mergeMap((address) => {
              ...
              if (
                ...
                payload.address.region.isocodeShort
              ) {
    ...
    this.actions$.pipe(
      ...
      mergeMap((payload) => {
        return this.checkoutDeliveryConnector
          ...
          .pipe(
            map((data) => {
              return new CheckoutActions.LoadSupportedDeliveryModesSuccess(
                data
              );
            }),
            catchError((error) =>
  ...
  > = createEffect(() =>
    this.actions$.pipe(
      ...
      map(
        (action: CheckoutActions.ClearCheckoutDeliveryMode) => action.payload
      ),
      filter((payload) => Boolean(payload.cartId)),
      concatMap((payload) => {
        return this.checkoutConnector
      ...
      switchMap((payload) => {
        return this.checkoutCostCenterConnector
          ...
          .pipe(
            ...
            catchError((error) =>
              of(
                new CheckoutActions.SetCostCenterFail(normalizeHttpError(error))
              )
            )
          );
      }),
