...
import { OrderHistoryExtendedService, OrderHistoryService } from '@spartacus/order/core';
...
export class OrderHistoryExtendedComponent extends OrderHistoryComponent {
  protected service = inject(OrderHistoryExtendedService);
  protected service1 = inject(OrderHistoryService);
  ...
  pageChange(page: number): void {
    ...
    this.service1.clearOrderList();
    const event: { sortCode: string; currentPage: number } = {
      sortCode: this.sortType,
      currentPage: page,
    };
    this.service.loadOrderList(
      this.ITEMS_PER_PAGE,
      event.currentPage,
      event.sortCode
    );
