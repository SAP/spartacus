...
export class ReplenishmentOrderDetailsEffect {
  @Effect()
  loadReplenishmentOrderDetails$: Observable<OrderActions.ReplenishmentOrderDetailsAction> = this.actions$.pipe(
    ofType(OrderActions.LOAD_REPLENISHMENT_ORDER_DETAILS),
    map((action: OrderActions.LoadReplenishmentOrderDetails) => action.payload),
    switchMap((payload) => {
  ...
  @Effect()
  cancelReplenishmentOrder$: Observable<OrderActions.ReplenishmentOrderDetailsAction> = this.actions$.pipe(
