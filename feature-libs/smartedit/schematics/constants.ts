import { SPARTACUS_SMARTEDIT } from '@spartacus/schematics';

export const SMARTEDIT_FOLDER_NAME = 'smartedit';
export const SMARTEDIT_MODULE = 'SmartEditModule';
export const SMARTEDIT_FEATURE_NAME = 'smartEdit';
export const SMARTEDIT_FEATURE_CONSTANT = 'SMART_EDIT_FEATURE';
export const SMARTEDIT_ROOT_MODULE = 'SmartEditRootModule';
export const SPARTACUS_SMARTEDIT_ROOT = `${SPARTACUS_SMARTEDIT}/root`;
export const SPARTACUS_SMARTEDIT_ASSETS = 'smartedit/assets';
