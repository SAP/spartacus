...
import { of } from "rxjs";
import { AccountSummaryDetails, AccountSummaryList, DocumentQueryParams, DocumentStatus, FilterByOptions } from "../../root/model";
...
class MockAccountSummaryAdapter implements AccountSummaryAdapter {
    getAccountSummary = createSpy('getAccountSummary').and.returnValue(of(accountSummaryResult));
    getDocumentList = createSpy('getDocumentList').and.returnValue(of(accountSummaryDocumentsResult));
}
...
describe('AccountSummaryConnector', () => {
    ...
    it('it should load account summary documents', () => {
        ...
        expect(adapter.getDocumentList).toHaveBeenCalledWith(userId, unitCode, queryParams);
    });
