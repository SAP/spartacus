...
import { CurrentUnitService, ItemService, UnitFormService } from '@spartacus/organization/administration/components';
...
export class AccountSummaryItemService extends ItemService<B2BUnit> {
  ...
  launchDetails(item: B2BUnit): void {
    ...
    if (detailsPageRoute && item && Object.keys(item).length > 0) {
      this.routingService.go({ cxRoute: detailsPageRoute, params: selectedUnit });
    }
  }

  protected create(_value: B2BUnit): Observable<OrganizationItemStatus<B2BUnit>> {
    throw new Error('Method not implemented.');
  }
  update(_key: string, _value: B2BUnit): Observable<OrganizationItemStatus<B2BUnit>> {
    throw new Error('Method not implemented.');
  }
