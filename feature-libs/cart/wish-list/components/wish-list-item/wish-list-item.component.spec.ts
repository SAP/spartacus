...
describe('WishListItemComponent', () => {
  ...
  it('should not display actions when entry is not updateable', () => {
    ...
    expect(el.query(By.css('button.link.cx-action-link'))).toBeNull();
    component.cartEntry.updateable = true;
  });

  it('should call remove', () => {
    spyOn(component, 'removeEntry');
    el.query(By.css('button.link.cx-action-link')).nativeElement.click();
  ...
  it('should disable remove link when loading', () => {
    ...
    expect(
      el.query(By.css('button.link.cx-action-link')).nativeElement.disabled
    ).toBeTruthy();
  });
