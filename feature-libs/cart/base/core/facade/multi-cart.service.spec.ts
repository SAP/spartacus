...
const testCart: Cart = {
  ...
  entries: [
    ...
    { entryNumber: 2, product: { code: '1236' }, inBundle: true},
  ],
  entryGroups: [
    {
      entryGroupNumber: 1,
...
describe('MultiCartService', () => {
  ...
  describe('getStandaloneEntries', () => {
    it('should return cart standalone entries', () => {
      let result;
      service.getStandaloneEntries('xxx').subscribe((cart) => {
        result = cart;
      });
      ...
      expect(result).toEqual(testCart.entries?.slice(0, 2));
    });
  });

  describe('getBundleEntryGroups', () => {
    it('should return cart bundle entry groups', () => {
      let result;
      service.getBundleEntryGroups('xxx').subscribe((cart) => {
        result = cart;
      });
