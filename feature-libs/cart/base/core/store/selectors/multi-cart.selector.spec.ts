...
describe('Multi Cart selectors', () => {
  ...
  const testCart: Cart = {
    ...
    entries: [
      ...
      { entryNumber: 3, product: { code: '3235' }, inBundle: true},
    ],
    entryGroups: [
      {
        entryGroupNumber: 1,
  ...
  describe('getStandaloneCartEntriesSelectorFactory', () => {
    it('should return cart standalone entries', () => {
      let result;
      store
        .pipe(
          select(
            MultiCartSelectors.getStandaloneCartEntriesSelectorFactory(testCart.code!)
          )
        )
        .subscribe((value) => (result = value));
      ...
      expect(result).toEqual(testCart.entries?.slice(0, 3));
    });
  });
