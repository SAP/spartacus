...
import { LoggerService, OccEndpointsService, tryNormalizeHttpError } from '@spartacus/core';
...
export class OccSubscriptionBillingAdapter implements SubscriptionBillingAdapter {
    ...
    getSubscriptionBillByCode(userId: string, billId: string): Observable<SubscriptionBill> {
        const url = this.occEndpoints.buildUrl('subscriptionBillsList', {
            urlParams: {
                userId,
                billId
