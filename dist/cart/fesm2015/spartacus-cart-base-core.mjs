...
class LoadCartsSuccess extends StateUtils.EntitySuccessAction {
    constructor(payload) {
        super(MULTI_CART_DATA, payload.map((cart) => { var _a; return (_a = cart === null || cart === void 0 ? void 0 : cart.code) !== null && _a !== void 0 ? _a : ''; }));
        this.payload = payload;
    ...
    constructor(actions$, cartEntryConnector) {
        ...
        this.addEntry$ = createEffect(() => this.actions$.pipe(ofType(CART_ADD_ENTRY), map((action) => action.payload), concatMap((payload) => {
            return this.cartEntryConnector
                ...
                new CartAddEntryFail(Object.assign(Object.assign({}, payload), { error: normalizeHttpError(error, this.logger) })),
                new LoadCart({
        ...
        this.removeEntry$ = createEffect(() => this.actions$.pipe(ofType(CART_REMOVE_ENTRY), map((action) => action.payload), concatMap((payload) => this.cartEntryConnector
            ...
            new CartRemoveEntryFail(Object.assign(Object.assign({}, payload), { error: normalizeHttpError(error, this.logger) })),
            new LoadCart({
                cartId: payload.cartId,
                userId: payload.userId,
            }),
        ...
        this.updateEntry$ = createEffect(() => this.actions$.pipe(ofType(CART_UPDATE_ENTRY), map((action) => action.payload), concatMap((payload) => this.cartEntryConnector
            ...
            new CartUpdateEntryFail(Object.assign(Object.assign({}, payload), { error: normalizeHttpError(error, this.logger) })),
            new LoadCart({
                cartId: payload.cartId,
                userId: payload.userId,
            }),
    ...
    constructor(actions$, cartVoucherConnector, messageService) {
        ...
        this.addCartVoucher$ = createEffect(() => this.actions$.pipe(ofType(CART_ADD_VOUCHER), map((action) => action.payload), mergeMap((payload) => {
            return this.cartVoucherConnector
                ...
                new CartAddVoucherFail(Object.assign(Object.assign({}, payload), { error: normalizeHttpError(error, this.logger) })),
...
function getCartIdByUserId(cart, userId) {
    ...
    if (userId === OCC_USER_ID_ANONYMOUS) {
        return (_a = cart === null || cart === void 0 ? void 0 : cart.guid) !== null && _a !== void 0 ? _a : '';
    }
    return (_b = cart === null || cart === void 0 ? void 0 : cart.code) !== null && _b !== void 0 ? _b : '';
}
/**
...
const getCartEntrySelectorFactory = (cartId, productCode) => {
    return createSelector(getCartEntriesSelectorFactory(cartId), (state) => {
        return state.find((entry) => { var _a; return ((_a = entry.product) === null || _a === void 0 ? void 0 : _a.code) === productCode; });
    });
};
...
class CartEffects {
    handleLoadCartError(payload, error) {
        ...
        return of(new LoadCartFail(Object.assign(Object.assign({}, payload), { error: normalizeHttpError(error, this.logger) })));
    }
    constructor(actions$, cartConnector, store) {
        ...
        this.createCart$ = createEffect(() => this.actions$.pipe(ofType(CREATE_CART), map((action) => action.payload), mergeMap((payload) => {
            ...
            }), catchError((error) => of(new CreateCartFail(Object.assign(Object.assign({}, payload), { error: normalizeHttpError(error, this.logger) })))));
        }), withdrawOn(this.contextChange$)));
        ...
        this.addEmail$ = createEffect(() => this.actions$.pipe(ofType(ADD_EMAIL_TO_CART), map((action) => action.payload), mergeMap((payload) => this.cartConnector
            ...
            new AddEmailToCartFail(Object.assign(Object.assign({}, payload), { error: normalizeHttpError(error, this.logger) })),
            new LoadCart({
                userId: payload.userId,
                cartId: payload.cartId,
            }),
        ...
        this.deleteCart$ = createEffect(() => this.actions$.pipe(ofType(DELETE_CART), map((action) => action.payload), mergeMap((payload) => this.cartConnector.delete(payload.userId, payload.cartId).pipe(map(() => {
            ...
            new DeleteCartFail(Object.assign(Object.assign({}, payload), { error: normalizeHttpError(error, this.logger) })),
...
class MultiCartEffectsService {
    ...
    getActiveCartTypeOnLoadSuccess(action) {
        ...
        if ((_b = (_a = action === null || action === void 0 ? void 0 : action.payload) === null || _a === void 0 ? void 0 : _a.extraData) === null || _b === void 0 ? void 0 : _b.active) {
            ...
            if ((_c = action.payload) === null || _c === void 0 ? void 0 : _c.cart.saveTime) {
...
class MultiCartEffects {
    ...
    getActiveCartTypeOnLoad(action) {
        var _a;
        if (((_a = action === null || action === void 0 ? void 0 : action.payload) === null || _a === void 0 ? void 0 : _a.cartId) === OCC_CART_ID_CURRENT) {
            return new SetCartTypeIndex({
                cartType: CartType.ACTIVE,
                cartId: '',
            });
        ...
        this.setActiveCartId$ = createEffect(() => this.actions$.pipe(ofType(LOAD_CART_SUCCESS, LOAD_CART, CREATE_CART_SUCCESS, CREATE_CART, SET_ACTIVE_CART_ID), map((action) => {
            ...
            switch (action.type) {
                ...
                case CREATE_CART_SUCCESS: {
                    return new SetCartTypeIndex({
                        cartType: ((_b = (_a = action === null || action === void 0 ? void 0 : action.payload) === null || _a === void 0 ? void 0 : _a.extraData) === null || _b === void 0 ? void 0 : _b.active)
                            ? CartType.ACTIVE
...
function cartTypeIndexReducer(state = cartTypeIndexInitialState, action) {
    ...
    switch (action.type) {
        case SET_CART_TYPE_INDEX:
            return Object.assign(Object.assign({}, state), { [action.payload.cartType]: action.payload.cartId });
        case REMOVE_CART:
        case DELETE_CART_SUCCESS: {
            if (((_a = action.payload) === null || _a === void 0 ? void 0 : _a.cartId) === state[CartType.ACTIVE]) {
                return Object.assign(Object.assign({}, state), { [CartType.ACTIVE]: '' });
            }
            return state;
        }
...
class MultiCartStatePersistenceService {
    ...
    getCartState() {
        ...
        filter((store) => !!store.cart), select(getMultiCartState), filter((state) => !!state), map((state) => state.index), distinctUntilKeyChanged('Active'), map((indexState) => {
            var _a;
            return {
                active: (_a = indexState[CartType.ACTIVE]) !== null && _a !== void 0 ? _a : '',
            };
        }));
    }
    onRead(state) {
...
function uninitializeActiveCartMetaReducerFactory() {
    const metaReducer = (reducer) => (state, action) => {
        ...
        if (action.type === '@ngrx/store/init') {
            newState.cart = Object.assign(Object.assign({}, newState.cart), { index: { [CartType.ACTIVE]: undefined } });
        }
...
class CartEventBuilder {
    ...
    registerDeleteCart() {
        this.stateEventService.register({
            ...
            factory: (action) => createFrom(DeleteCartEvent, Object.assign(Object.assign({}, action.payload), { cartCode: action.payload.cartId })),
        });
        this.stateEventService.register({
            action: DELETE_CART_SUCCESS,
            event: DeleteCartSuccessEvent,
            factory: (action) => createFrom(DeleteCartSuccessEvent, Object.assign(Object.assign({}, action.payload), { cartCode: action.payload.cartId })),
        });
        this.stateEventService.register({
            action: DELETE_CART_FAIL,
            event: DeleteCartFailEvent,
            factory: (action) => createFrom(DeleteCartFailEvent, Object.assign(Object.assign({}, action.payload), { cartCode: action.payload.cartId })),
        });
    }
    ...
    registerMapped(mapping) {
        const eventStream$ = this.getAction(mapping.action).pipe(switchMap((action) => {
        ...
        }), filter(([action, _activeCart, activeCartId]) => action.payload['cartId'] === activeCartId), map(([action, activeCart]) => {
            var _a;
            return createFrom(mapping.event, Object.assign(Object.assign({}, action.payload), { cartCode: activeCart.code, entry: action.payload.entry
                    ...
                    : (_a = activeCart.entries) === null || _a === void 0 ? void 0 : _a[Number(action.payload.entryNumber)] }));
        }));
...
class ActiveCartService {
    ...
    detectUserChange() {
        ...
        if (this.isLoggedInWithCodeFlow()) {
            ...
            this.subscription.add(this.userIdService
                ...
                (_b = (_a = this.winRef) === null || _a === void 0 ? void 0 : _a.localStorage) === null || _b === void 0 ? void 0 : _b.removeItem(OAUTH_REDIRECT_FLOW_KEY);
            }));
        }
    }
    /**
    ...
    addEntriesGuestMerge(cartEntries) {
        const entriesToAdd = cartEntries.map((entry) => {
            var _a, _b, _c;
            return ({
                productCode: (_b = (_a = entry.product) === null || _a === void 0 ? void 0 : _a.code) !== null && _b !== void 0 ? _b : '',
                quantity: (_c = entry.quantity) !== null && _c !== void 0 ? _c : 0,
            });
        });
    ...
    isLoggedInWithCodeFlow() {
        var _a, _b;
        return !!((_b = (_a = this.winRef) === null || _a === void 0 ? void 0 : _a.localStorage) === null || _b === void 0 ? void 0 : _b.getItem(OAUTH_REDIRECT_FLOW_KEY));
    }
    ...
    isCartUserGuest(cart) {
        ...
        return Boolean(cartUser &&
            (cartUser.name === OCC_USER_ID_GUEST ||
                isEmail((_a = cartUser.uid) === null || _a === void 0 ? void 0 : _a.split('|').slice(1).join('|'))));
    }
    /**
...
i0.ɵɵngDeclareClassMetadata({ minVersion: "12.0.0", version: "15.2.9", ngImport: i0, type: ActiveCartService, decorators: [{
        ...
        return [{ type: i1$2.MultiCartFacade }, { type: i2.UserIdService }, { type: i2.WindowRef, decorators: [{
                        type: Optional
                    }] }];
    } });
...
class MultiCartService {
    ...
    createCart({ userId, oldCartId, toMergeCartGuid, extraData, }) {
        ...
        return this.getCartIdByType((extraData === null || extraData === void 0 ? void 0 : extraData.active) ? CartType.ACTIVE : CartType.NEW_CREATED).pipe(switchMap((cartId) => this.getCart(cartId)), filter(isNotUndefined));
    }
    /**
    ...
    getLastEntry(cartId, productCode) {
        return this.store.pipe(select(getCartEntriesSelectorFactory(cartId)), map((entries) => {
            const filteredEntries = entries.filter((entry) => { var _a; return ((_a = entry.product) === null || _a === void 0 ? void 0 : _a.code) === productCode; });
    ...
    canActivate() {
        return !this.cartConfigService.isCartValidationEnabled()
            ...
            : this.cartValidationService.validateCart().pipe(withLatestFrom(this.activeCartService.getEntries()), map(([cartModificationList, cartEntries]) => {
                ...
                this.cartValidationStateService.updateValidationResultAndRoutingId((_a = cartModificationList.cartModifications) !== null && _a !== void 0 ? _a : []);
                if (cartModificationList.cartModifications !== undefined &&
                    ...
                    if (cartEntries.length === 1 &&
                        ((_b = cartEntries[0].product) === null || _b === void 0 ? void 0 : _b.code) ===
                            ((_d = (_c = modification.entry) === null || _c === void 0 ? void 0 : _c.product) === null || _d === void 0 ? void 0 : _d.code) &&
                        modification.statusCode === CartValidationStatusCode.NO_STOCK) {
                        validationResultMessage = {
                            key: 'validation.cartEntryRemoved',
                            params: {
                                name: (_f = (_e = modification.entry) === null || _e === void 0 ? void 0 : _e.product) === null || _f === void 0 ? void 0 : _f.name,
                            },
                        };
                    }
                    else {
                    ...
                    return this.router.parseUrl((_g = this.semanticPathService.get('cart')) !== null && _g !== void 0 ? _g : '');
                }
                return true;
            }));
    }
