...
class OccCartNormalizer {
    ...
    convert(source, target) {
        ...
        if (source.entries) {
            target.entries = source.entries.map((entry) => (Object.assign(Object.assign({}, entry), { product: this.converter.convert(entry.product, PRODUCT_NORMALIZER), promotions: this.converter.convert({ item: entry, promotions: target === null || target === void 0 ? void 0 : target.appliedProductPromotions }, ORDER_ENTRY_PROMOTIONS_NORMALIZER) })));
        }
        return target;
    }
    /**
...
class OrderEntryPromotionsNormalizer {
    ...
    getProductPromotion(item, promotions) {
        ...
        promotions === null || promotions === void 0 ? void 0 : promotions.forEach((promotion) => {
...
class OccCartEntryAdapter {
    ...
    add(userId, cartId, productCode, quantity = 1, pickupStore) {
        ...
        const toAdd = Object.assign({ quantity, product: { code: productCode } }, (pickupStore && { deliveryPointOfService: { name: pickupStore } }));
...
class OccCartAdapter {
    ...
    loadAll(userId) {
        return this.http
            ...
            .pipe(map((cartList) => { var _a; return (_a = cartList.carts) !== null && _a !== void 0 ? _a : []; }), this.converterService.pipeableMany(CART_NORMALIZER));
    }
    load(userId, cartId) {
