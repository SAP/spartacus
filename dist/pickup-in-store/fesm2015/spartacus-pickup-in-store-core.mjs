...
class DefaultPointOfServiceEffect {
    constructor(actions$, store, userProfileService, winRef) {
        ...
        this.loadDefaultPointOfService$ = createEffect(() => this.actions$.pipe(ofType(LOAD_DEFAULT_POINT_OF_SERVICE), switchMap(() => this.userProfileService.get().pipe(mergeMap((preferredStore) => iif(() => !!preferredStore && !!preferredStore.defaultPointOfServiceName, of({
            ...
            var _a;
            const PREFERRED_STORE = (_a = this.winRef.localStorage) === null || _a === void 0 ? void 0 : _a.getItem(PREFERRED_STORE_LOCAL_STORAGE_KEY);
        ...
        this.setDefaultPointOfService$ = createEffect(() => this.actions$.pipe(ofType(SET_DEFAULT_POINT_OF_SERVICE), map((action) => action['payload']), tap((preferredStore) => { var _a; return (_a = this.winRef.localStorage) === null || _a === void 0 ? void 0 : _a.setItem(PREFERRED_STORE_LOCAL_STORAGE_KEY, JSON.stringify(preferredStore)); }), switchMap((preferredStore) => this.userProfileService
            .update({
...
class StockEffect {
    constructor(actions$, stockConnector) {
        ...
        this.loadStockLevels$ = createEffect(() => this.actions$.pipe(ofType(STOCK_LEVEL), map((action) => action.payload), switchMap((_a) => {
            var { productCode } = _a, location = __rest(_a, ["productCode"]);
...
const intendedPickupLocationsReducer = createReducer(intendedPickupLocationsInitialState, on(AddLocation, (state, { payload }) => (Object.assign(Object.assign({}, state), { [payload.productCode]: payload.location }))), on(RemoveLocation, (state, { payload }) => (Object.assign(Object.assign({}, state), { [payload]: { pickupOption: 'delivery' } }))), on(SetPickupOption$1, (state, { payload }) => (Object.assign(Object.assign({}, state), { [payload.productCode]: Object.assign(Object.assign({}, state[payload.productCode]), { pickupOption: payload.pickupOption }) }))));
...
const storeDetailsReducer = createReducer(storeDetailsInitialState, on(SetStoreDetailsSuccess, (state, { payload }) => (Object.assign(Object.assign({}, state), (payload.name ? { [payload.name]: payload } : {})))));
...
const pickupOptionReducer = createReducer(initialState$2, on(SetPickupOption, (state, { payload }) => {
    ...
    return state
        ...
        .map((entry) => (Object.assign(Object.assign({}, entry), { entryNumber: entry.entryNumber > payload.entryNumber
            ? entry.entryNumber - 1
            : entry.entryNumber })));
...
const browserLocationReducer = createReducer(initialState$1, on(AddBrowserLocation, (state, { payload }) => (Object.assign(Object.assign({}, state), { latitude: payload.latitude, longitude: payload.longitude }))));
...
const _stockReducer = createReducer(initialStockLevelState, on(createAction(STOCK_LEVEL_SUCCESS, props()), (state, { payload: { productCode, stockLevels } }) => (Object.assign(Object.assign({}, state), { [productCode]: stockLevels }))), on(createAction(CLEAR_STOCK_DATA), (_state) => ({})));
function stockReducer(state, action) {
    return _stockReducer(state, action);
}
const initialStockLevelAtStoreState = {};
const stockAtStoreReducer = createReducer(initialStockLevelAtStoreState, on(StockLevelAtStoreSuccess, (state, { payload }) => (Object.assign(Object.assign({}, state), { [payload.productCode]: { [payload.storeName]: payload.stockLevel } }))));
...
const getPickupOptionByProductCode = (productCode) => createSelector(getIntendedPickupLocationByProductCode(productCode), (_getIntendedPickupLocationByProductCode) => { var _a; return (_a = _getIntendedPickupLocationByProductCode === null || _getIntendedPickupLocationByProductCode === void 0 ? void 0 : _getIntendedPickupLocationByProductCode.pickupOption) !== null && _a !== void 0 ? _a : 'delivery'; });
...
const getPickupOption = (entryNumber) => createSelector(getPickupOptionState, (state) => {
    var _a;
    return (_a = state.pickupOption.find((entry) => entry.entryNumber === entryNumber)) === null || _a === void 0 ? void 0 : _a.pickupOption;
});
...
const getStoresWithStockForProductCode = (productCode) => createSelector(getStockEntities, getHideOutOfStockState, (stockEntities, hideOutOfStock) => { var _a, _b, _c; return (_c = (_b = (_a = stockEntities[productCode]) === null || _a === void 0 ? void 0 : _a.stores) === null || _b === void 0 ? void 0 : _b.filter((store) => !hideOutOfStock || storeHasStock(store))) !== null && _c !== void 0 ? _c : []; });
const getStockAtStore = (productCode, storeName) => createSelector(getStockState, (stockState) => { var _a, _b; return (_b = (_a = stockState === null || stockState === void 0 ? void 0 : stockState.stockLevelAtStore) === null || _a === void 0 ? void 0 : _a[productCode]) === null || _b === void 0 ? void 0 : _b[storeName]; });
...
class PreferredStoreService {
    ...
    clearPreferredStore() {
        var _a;
        (_a = this.winRef.localStorage) === null || _a === void 0 ? void 0 : _a.removeItem(PREFERRED_STORE_LOCAL_STORAGE_KEY);
    }
    /**
