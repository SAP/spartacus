...
- infinite loop on the payment step
- cost center shipping address doing 2 calls to set the same address
- `orderType$` in `CheckoutScheduledReplenishmentService` - should it be ORDER_TYPE.SCHEDULE_REPLENISHMENT_ORDER?
  - check its usages as well
