...
It's also important to note that after we release a _next.x_, or an _rc.x_ version, all the migration scripts that are written after the release _have_ to specify the future release version.
E.g. if _2.0.0-next.1_ has been released, then the future migration scripts should specify _2.0.0-next.2_.
This is required for clients that upgrade frequently and it will run only the new migration scripts for them. At the same time, all the scripts are going to be ran for the clients that do the upgrade for the first time. As we have data-driven generic mechanisms for e.g. constructor deprecation, we have to bump the version for those scripts as well.
