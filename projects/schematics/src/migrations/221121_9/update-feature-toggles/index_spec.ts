...
describe('Update feature toggles migration', () => {
  ...
  it('should comment out unknown toggles with ✅ marker', async () => {
    ...
    // Unknown toggles should be commented out with ✅
    expect(content).toContain('// ✅');
    expect(content).toMatch(/\/\/ ✅.*"toggleOld": true,/);
    expect(content).toMatch(/\/\/ ✅.*"toggleRemoved": true,/);
...
`
  ...
  it('should comment out ALL unknown toggles when none are valid', async () => {
    ...
    const content = newTree.readText(MODULE_FILE);
    expect(content).toMatch(/\/\/ ✅.*"oldToggle1"/);
    expect(content).toMatch(/\/\/ ✅.*"oldToggle2"/);
  });
  ...
  it('should find the module file in alternative locations', async () => {
    ...
    expect(content).toContain('"toggleA": true,');
    expect(content).toMatch(/\/\/ ✅.*"toggleOld"/);
  });
});
