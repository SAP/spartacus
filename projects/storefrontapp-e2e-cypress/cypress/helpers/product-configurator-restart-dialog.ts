...
export function checkIsOpen() {
  cy.get('cx-configurator-restart-dialog').should('be.visible');
}
...
export function checkIsClosed() {
  cy.get('cx-configurator-restart-dialog').should('not.exist');
}
...
/**
 * closes the restart dialog via the close button
 */
export function close(productId: string) {
  cy.get(COMPONENT_SELECTOR).within(() => cy.get('button.close').click());
  const location = `/${Cypress.env('BASE_SITE')}/en/USD/product/${productId}/`;
