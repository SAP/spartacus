    ...
    it('should be able to remove coupon from cart (CXSPA-3906)', () => {
      cy.get('.cx-asm-customer-promotion-listing-row')
        ...
        .within(() => {
          ...
          // cy.intercept('POST', /\.*\/customer360\.*/).as('fetchData2');
          // cy.wait('@fetchData2').its('response.statusCode').should('eq', 200);
