...
describe('Profile-tag events', () => {
  ...
  describe('cart events', () => {
    it(
      ['cds', 'events', 'profile-tag'],
      'should send a AddedToCart event on adding an item to cart',
      () => {
        goToProductPage();
    ...
    it(
      ['cds', 'events', 'profile-tag'],
      'should send a CartModified event on modifying the cart',
      () => {
        goToProductPage();
        cy.intercept({
          method: 'GET',
    ...
    it(
      ['cds', 'events', 'profile-tag'],
      'should send a RemovedFromCart event on removing an item from the cart',
      () => {
        goToProductPage();
        cy.get('cx-add-to-cart button.btn-primary').click();
        cy.get('cx-added-to-cart-dialog .btn-primary').click();
  ...
  it(
    ['cds', 'events', 'profile-tag'],
    'should send a product detail page view event when viewing a product',
    () => {
      cy.intercept({ method: 'GET', path: `**reviews*` }).as('lastRequest');
      const productSku = 280916;
      const productName =
        'Web Camera (100KpixelM CMOS, 640X480, USB 1.1) Black';
      ...
      cy.window().should((win) => {
        ...
        expect(productViewedEvent.data.productCategory).to.equal(
          productCategory
        );
  ...
  it(
    ['cds', 'events', 'profile-tag'],
    'should send a search page view event when viewing a search page',
    () => {
  ...
  it(
    ['cds', 'events', 'profile-tag'],
    'should send a CartPageViewed event when viewing the cart page',
    () => {
      goToProductPage();
      cy.get('cx-add-to-cart button.btn-primary').click();
      cy.get('cx-added-to-cart-dialog .btn-primary').click();
  ...
  it(
    ['cds', 'events', 'profile-tag'],
    'should send an OrderConfirmation event when viewing the order confirmation page',
    () => {
  ...
  it(
    ['cds', 'events', 'profile-tag'],
    'should send a Category View event when a Category View occurs',
    () => {
  ...
  it(
    ['cds', 'events', 'profile-tag'],
    'should send 2 Category Views event when going to a Category, going to a different page type, and then back to the same category',
    () => {
      cy.intercept({ method: 'GET', path: `**/products/search**` }).as(
        'lastRequest'
      );
  ...
  it(
    ['cds', 'events', 'profile-tag'],
    'should send 1 Category View event when going to a Category and clicking a facet',
    () => {
      cy.intercept({ method: 'GET', path: `**/products/search**` }).as(
        'lastRequest'
      );
  ...
  it(
    ['cds', 'events', 'profile-tag'],
    'should send a Navigated event when a navigation to product page occurs',
    () => {
      goToProductPage();
      cy.get('cx-add-to-cart button.btn-primary').click();
      cy.window().should((win) => {
        expect(
          profileTagHelper.eventCount(
            win,
            profileTagHelper.EventNames.NAVIGATED
          )
        ).to.equal(1);
      });
    }
  ...
  it(
    ['cds', 'events', 'profile-tag'],
    'should not send a Navigated event when merchandising banner is clicked',
    () => {
      const categoryPage = checkoutFlow.waitForCategoryPage(
        '578',
        'getCategory'
      );
      cy.get(
...
describe('Consent Changed', () => {
  ...
  it(
    ['cds', 'events', 'profile-tag'],
    'should send a consentGranted = false before accepting consent, and a consentGranted=true after accepting',
    () => {
      cy.wait(2000);
      cy.window()
        .should((win) => {
  ...
  it(
    ['cds', 'events', 'profile-tag'],
    'should not send a consentgranted=false event on a page refresh',
    () => {
      anonymousConsents.clickAllowAllFromBanner();
      cy.reload();
      cy.window().should((win) => {
  ...
  it(
    ['cds', 'events', 'profile-tag'],
    'should send a HomePageViewed event when viewing the homepage',
    () => {
      anonymousConsents.clickAllowAllFromBanner();
      cy.reload();
...
describe('verifying X-Consent-Reference header addition to occ calls', () => {
  ...
  it(
    ['cds', 'events', 'profile-tag'],
    'should not send CR header when consent is not granted initially',
    () => {
  ...
  it(
    ['cds', 'events', 'profile-tag'],
    'should send CR header when consent is granted and skip it once its revoked',
    () => {
      // grant consent
      anonymousConsents.clickAllowAllFromBanner();
