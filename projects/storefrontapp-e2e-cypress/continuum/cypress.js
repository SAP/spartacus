...
const submitAccessibilityConcernsToAMP = () => {
  ...
  cy.title({ log: false }).then((pageTitle) => {
    cy.url({ log: false }).then({ timeout: 30000 }, async (pageUrl) => {
      ...
      await ampReportingService.setActiveOrganization(10039); // ID of AMP organization to submit test results to
      await ampReportingService.setActiveAsset(38419); // ID of AMP asset to submit test results to
