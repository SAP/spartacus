...
export class ProductsSearchEffects {
  ...
  getProductsList$: Observable<
    ProductActions.GetProductsListSuccess | ProductActions.GetProductsListFail
  > = createEffect(() =>
    this.actions$.pipe(
      ofType(ProductActions.GET_PRODUCTS_LIST),
      map((action: ProductActions.GetProductsList) => action.payload),
      mergeMap((payload) => {
        return this.productSearchConnector.searchByCodes(payload.codeList).pipe(
          map((data) => {
            return new ProductActions.GetProductsListSuccess({
              results: data,
