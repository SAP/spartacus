...
describe('Product Reducer', () => {
  ...
  describe('LOAD_PRODUCT_SUCCESS action', () => {
    it('should populate product details entities', () => {
      ...
      expect(state).toEqual({
        testCode: { loading: false, value: product, error: false }
      });
    });
  });
  ...
  describe('LOAD_PRODUCT_FAIL action', () => {
    it('should return an empty product', () => {
      ...
      expect(stateAfterSuccess).toBeTruthy();

      expect(stateAfterSuccess[code]).toEqual({
        loading: false,
        error: false,
        value: product
      });
      ...
      expect(stateAfterFail[code]).toEqual({
        loading: false,
        error: true,
        value: null
      });
