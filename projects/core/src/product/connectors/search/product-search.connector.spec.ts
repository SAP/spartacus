...
class MockProductSearchAdapter implements ProductSearchAdapter {
  ...
  searchByCodes = createSpy('ProductSearchAdapter.searchByCodes').and.callFake((codes, scope) =>
    of({ products: codes.map(code => ({ code, scope })) })
  );
}
...
describe('ProductSearchConnector', () => {
  ...
  it('search should call adapter', () => {
    ...
    expect(adapter.search).toHaveBeenCalledWith('test query', undefined, undefined);
  });

  it('searchByCodes should call adapter', () => {
    let result;
    service.searchByCodes(['code1', 'code2']).subscribe((res) => (result = res));
    expect(result).toEqual({ products: [{ code: 'code1', scope: undefined }, { code: 'code2', scope: undefined }] });
    expect(adapter.searchByCodes).toHaveBeenCalledWith(['code1', 'code2'], undefined);
  });
