...
Some Storefront's routes already have predefined `paramsMapping`. They can be found in [`default-routing-config.ts`]
(storefrontlib\src\lib\ui\pages\default-routing-config.ts).
