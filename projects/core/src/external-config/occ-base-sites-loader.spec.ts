...
describe(`OccBaseSitesLoader`, () => {
  describe(`load`, () => {
    ...
    it(`should call GET xhr with default URL parts`, async () => {
      ...
      expect(LoadJsonUtils.loadXhr).toHaveBeenCalledWith(
        'https://base.com/rest/v2/basesites?fields=baseSites(uid,defaultLanguage(isocode),urlEncodingAttributes,urlPatterns,stores(currencies(isocode),defaultCurrency(isocode),languages(isocode),defaultLanguage(isocode)))'
      );
    });
