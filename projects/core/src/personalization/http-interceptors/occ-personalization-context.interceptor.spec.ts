...
describe('OccPersonalizationContextInterceptor', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      ...
      providers: [
        ...
        { provide: PersonalizationContextService, useValue: personalizationContextService },
        {
  ...
  it('should set personalization context if PersonalizationScriptComponent exists in response', inject(
    ...
    (http: HttpClient) => {
      ...
      personalizationContextService.getPersonalizationContext().subscribe(value => {
        expect(value.segments[0]).toEqual("segment1");
        expect(value.segments[1]).toEqual("segment2");
        expect(value.actions[0].customization_code).toEqual("customization_code1");
        expect(value.actions[1].customization_code).toEqual("customization_code2");
      });
