...
function getStorefrontConfig(options: SpartacusOptions): string {
  ...
  const customBaseSite = !options.baseSite
    ? ''
    : `
          '${options.baseSite}',`;
  return `{
...
function updateAppModule(options: SpartacusOptions): Rule {
  return (host: Tree, context: SchematicContext) => {
    ...
    if (
      ...
      addImport(host, modulePath, TEST_CONFIG_MODULE, SPARTACUS_CORE);
      addImport(
        host,
        modulePath,
        TEST_OUTLET_MODULE,
      ...
      addToModuleImportsAndCommitChanges(host, modulePath, TEST_OUTLET_MODULE);

      addToModuleImportsAndCommitChanges(host, modulePath, TEST_CONFIG_MODULE);
    }
...
function provideTestOutletsModule(): Source {
...
export function addSpartacus(options: SpartacusOptions): Rule {
  return (tree: Tree, context: SchematicContext) => {
    ...
    return chain([
      ...
      mergeWith(provideTestOutletsModule(), MergeStrategy.Overwrite),
