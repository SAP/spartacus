import {browser, ElementFinder, ElementArrayFinder, by, element} from 'protractor';
...
export class CartPage extends AppPage {
  ...
  readonly cartEntries : ElementArrayFinder = this
    .page
    .all(by.tagName('cx-cart-item'));
  readonly orderSummary : ElementFinder = this
    .page
    .element(by.tagName('cx-order-summary'));
  readonly cartEntryByProductName = (productName : string) : ElementFinder => this
    .cartEntries
    .filter(el => el.element(by.css('.cx-cart-item__name')).getText().then(text => text === productName))
    .first();
  ...
  async getCartEntryUnitPrice(cartEntry : ElementFinder) : Promise < string > {
    const unitPriceDiv = cartEntry.element(by.css('.cx-cart-item__price--value'));
  ...
  async getCartEntryTotalPrice(cartEntry : ElementFinder) : Promise < string > {
    const totalPriceDiv = cartEntry.element(by.css('.cx-cart-item__total--value'));
    return E2EUtil.findPrice(await totalPriceDiv.getText());
  }

  async checkCartEntry(productName : string, quantity : string | number, unitPrice : string, totalPrice : string) {
    const product = this.cartEntryByProductName(productName);
    expect(await product.isDisplayed()).toBeTruthy();

    expect(parseInt(await this.getCartEntryQuantity(product), 10)).toBe(quantity, 'Wrong cart entry quantity');
  ...
  async getOrderSummaryInnerDivValue(textTitle : string) : Promise < string > {
    const outerDiv = this
      .orderSummary
      .element(by.css('div[class="order-summary"]'));
    const mainDiv = outerDiv
      .all(by.tagName('div'))
      .first();
  ...
  async checkCartSummary(subtotal : string, discount : string, total : string) {
    ...
    // in accelerator
    expect(await this.getSummarySubtotalValue()).toBe(subtotal, 'Wrong cart summary subtotal');

    expect(await this.getSummaryDiscountValue()).toBe(discount, 'Wrong cart summary discount');

    expect(await this.getSummaryTotalValue()).toBe(total, 'Wrong cart summary total');
  }
}
