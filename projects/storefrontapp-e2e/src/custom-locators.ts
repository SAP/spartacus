...
export function addCustomLocators(by : ProtractorBy) {
  by
    .addLocator('dynamicSlot', function (slotPosition, elementTag, opt_parentElement, _opt_rootSelector) {
      const using = opt_parentElement || document;
      return using.querySelectorAll(`cx-dynamic-slot[position=${slotPosition}] ${elementTag}`);
    });

  by.addLocator('formControlName', function (formControlName : string, opt_parentElement, _opt_rootSelector) {
    const using = opt_parentElement || document;
