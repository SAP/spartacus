...
export class CartPage extends AppPage {
  ...
  getCartEntryQty(i): ElementFinder {
  ...
  getCartEntryDecreaseQtyBtn(i): ElementFinder {
    return E2EUtil.getComponentWithinParent(
      this.getCartEntry(i),
      '.item-counter button.item-counter__decrease'
    );
  }

  getCartEntryIncreaseQtyBtn(i): ElementFinder {
    return E2EUtil.getComponentWithinParent(
      this.getCartEntry(i),
      '.item-counter button.item-counter__increase'
    );
  }

  getCartEntry(i): ElementFinder {
    return E2EUtil.getComponentsWithinParent(this.getPage(), 'y-cart-item').get(
      i
    );
  }

  getCartId(): ElementArrayFinder {
    return E2EUtil.getComponentsWithinParent(this.getPage(), 'y-cart-item');
  }

  getCartDetails(): ElementFinder {
    return E2EUtil.getComponentWithinParent(this.getPage(), 'y-cart-details');
  }

  getCartSubTitle(): promise.Promise<string> {
    const cartSubTitle = E2EUtil.getComponentWithinParentByCss(
      this.getCartDetails(),
      'div.sub-heading'
    );
    return cartSubTitle.getText().then(text => {
      return text;
    });
  }

  getCartTitle(): promise.Promise<string> {
    const cartTitle = E2EUtil.getComponentWithinParentByCss(
      this.getCartDetails(),
      'div.headings h2'
    );
    return cartTitle.getText().then(text => {
      return text;
    });
  }

  getShoppingCartId(): promise.Promise<string> {
    const cartTitle = E2EUtil.getComponentWithinParentByCss(
      this.getCartDetails(),
      'div.headings h4'
    );
  ...
  checkPageTitle() {
    this.getCartTitle().then(title => {
      expect(title).toBe('Shopping Cart');
    });
  }

  checkPageSubtitle(items, price) {
    this.getCartSubTitle().then(title => {
      expect(title).toBe(`Cart total (${items} items): ${price}`);
    });
  }

  checkCartId() {
    this.getShoppingCartId().then(title => {
