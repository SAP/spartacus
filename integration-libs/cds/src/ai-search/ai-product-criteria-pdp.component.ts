...
import { CurrentProductService, SearchBoxComponentService } from '@spartacus/storefront';
import {
  AiProductCriteria,
...
@Component({
  selector: 'cx-ai-product-criteria-pdp',
  templateUrl: './ai-product-criteria.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
  standalone: false,
})
export class AiProductCriteriaPdpComponent implements OnInit {
  ...
  readonly tooltipId = `cx-ai-criteria-pdp-tooltip-${++AiProductCriteriaPdpComponent.idCounter}`;
  ...
  ngOnInit(): void {
    this.criteria$ = combineLatest([
      ...
      switchMap(([lastSearchWasAi, query, product]) => {
