...
export class OccDpRequestNormalizer
  ...
  convert(
    ...
    target.sessionId = source?.parameters?.entry?.find((it: any) => it.key === 'session_id')?.value;
    target.signature = source?.parameters?.entry?.find((it: any) => it.key === 'signature')?.value;
    return target;
  }
}
