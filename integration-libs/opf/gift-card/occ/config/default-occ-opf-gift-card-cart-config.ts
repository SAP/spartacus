...
const giftCardCartEndpoints: CartOccEndpoints = {
  ...
  createCart:
    'users/${userId}/carts?fields=DEFAULT,sapGiftCards(FULL,balance(value,formattedValue),remainingBalance(value,formattedValue), appliedAmount(value,formattedValue)),sapGiftCardSummary(FULL,totalAppliedAmount(value,formattedValue),totalRemainingBalance(value,formattedValue), totalBalance(value,formattedValue)),potentialProductPromotions,appliedProductPromotions,potentialOrderPromotions,appliedOrderPromotions,entries(totalPrice(formattedValue),product(images(FULL),stock(FULL)),basePrice(formattedValue,value),updateable),totalPrice(formattedValue),totalItems,totalPriceWithTax(formattedValue),totalDiscounts(value,formattedValue),subTotal(formattedValue),totalUnitCount,deliveryItemsQuantity,deliveryCost(formattedValue),totalTax(formattedValue, value),pickupItemsQuantity,net,appliedVouchers,productDiscounts(formattedValue),user,_availableOperations',
};
